...
// LeonardoMotionResponse interface - kept for documentation of Leonardo API response structure
...
async function generateVideoWithLeonardo(
  ...
  const generationId =
    createData?.motionSvdGenerationJob?.generationId ||
  ...
  if (!generationId) {
    console.error(
      '[Leonardo] No generation ID found in any response field:',
      JSON.stringify(createData)
    );
    throw new Error(
      `Leonardo Motion API returned unexpected response: ${JSON.stringify(createData)}`
    );
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (!statusResponse.ok) {
      console.error(
        `[Leonardo] Status check failed (attempt ${attempt + 1}):`,
...
async function downloadAndUploadVideo(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const result = await supabase.storage.from('story-images').upload(fileName, videoBuffer, {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { imageUrl, prompt, storyId: _storyId, nodeId, resolution: _resolution = '480' } = body;
    ...
    if (nodeId) {
      await supabase.from('story_nodes').update({ video_url: publicVideoUrl }).eq('id', nodeId);
    }
