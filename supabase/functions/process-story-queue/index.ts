  ...
  try {
    ...
    if (requestedStoryId) {
      ...
      if (existingJob && existingJob.status === "processing") {
        return new Response(
          JSON.stringify({ error: "Story generation already in progress" }),
          {
            status: 200,
      ...
      if (existingJob) {
        ...
        return new Response(
          JSON.stringify({ error: "No queued job found for this story" }),
          {
            status: 404,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
      ...
      if (!nextJob) {
        return new Response(
          JSON.stringify({ message: "No pending stories to process" }),
          {
            status: 200,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
    ...
    if (!job) {
      return new Response(
        JSON.stringify({ error: "Failed to lock a generation job" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    const firstChapterSummary = startNode.chapter_summary ||
      `Chapter 1: ${startNode.content.slice(0, 100)}...`;
...
async function generateStoryTree(
  ...
  try {
    ...
    let memory = context?.memory || createInitialMemory(story, { content: '', chapter_summary: '' });
...
Write chapter ${node.chapterNumber} that directly follows from this choice.`;
        ...
        const isKidFriendly = await validateKidFriendlyContent(generatedContent.content, geminiApiKey);
        if (!isKidFriendly) {
        ...
        const chapterSummary = generatedContent.chapterSummary ||
        ...
        const { data: updatedNode, error: updateError } = await supabase
          ...
          .update({
            ...
            character_states: memory.characters.length > 0 ?
              Object.fromEntries(memory.characters.map(c => [c.name, c.appearance])) : null,
          })
...
function buildCoherentSystemPrompt(
  ...
  const characterContext = memory.characters.length > 0
    ? memory.characters.map(c => `${c.name}: ${c.appearance}, wearing ${c.clothing}`).join('\n')
    : 'Characters will be established in the story.';

  const eventsSummary = chapterSummaries.length > 0
    ? chapterSummaries.slice(-5).join('\n')
    : 'Story just beginning.';

  const unresolvedList = memory.unresolvedThreads.length > 0
...
` : isNearEnd ? `This is near the end. Start wrapping up conflicts and moving toward resolution.` : `Continue building the story.`}
