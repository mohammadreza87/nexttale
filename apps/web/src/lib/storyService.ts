...
// Helper to cast database JSON fields to proper types
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function castStoryTypes<T extends Record<string, any>>(story: T): T & Pick<Story, 'story_outline' | 'story_memory'> {
  return {
    ...story,
    story_outline: story.story_outline as StoryOutline | null | undefined,
    story_memory: story.story_memory as StoryMemory | null | undefined,
  };
}
...
export async function getStories(): Promise<Story[]> {
  ...
  const storiesWithCreators = await Promise.all(
    stories.map(async (story) => {
      ...
      return castStoryTypes({ ...story, cover_image_url: coverImage, creator, comment_count: commentCount || 0 });
    })
  );

  return storiesWithCreators as Story[];
}
...
export async function getStoriesPaginated(
  ...
  return {
    data: storiesWithCreators as Story[],
...
export async function getUserStoriesPaginated(
  ...
  const storiesWithCreators = await Promise.all(
    stories.map(async (story) => {
      ...
      return castStoryTypes({ ...story, cover_image_url: coverImage, creator });
    })
  );
