...
import type { FeedFilter as RepoFeedFilter } from '../data/types';
...
export async function getUnifiedFeed(
  ...
  // NEW: Use optimized repository pattern for unified feed
  if (featureFlags.isEnabled('USE_REPOSITORY_PATTERN')) {
    const result = await feedRepository.getUnifiedFeed(
      filter as RepoFeedFilter,
      limit,
      offset,
      excludeIds
    );

    // Transform repository result to FeedItem interface
    const feedItems: FeedItem[] = result.data.map((item) => ({
      id: item.id,
      title: item.title,
      description: item.description,
      feed_type: item.feed_type as ContentType,
      ...
      creator: item.creator_name
        ...
        : null,
      // Pass through extra_data for music/interactive content
      ...(item.extra_data && item.feed_type === 'music'
        ? {
            audio_url: (item.extra_data as { audio_url?: string }).audio_url,
            lyrics: (item.extra_data as { lyrics?: string }).lyrics,
            genre: (item.extra_data as { genre?: string }).genre,
            mood: (item.extra_data as { mood?: string }).mood,
          }
        : {}),
      ...(item.extra_data && item.feed_type !== 'story' && item.feed_type !== 'music'
        ? {
            tags: (item.extra_data as { tags?: string[] }).tags,
          }
        : {}),
    }));
  ...
  // LEGACY: Old implementation (will be removed after migration)
...
export async function getTrendingFeed(limit: number = 20): Promise<FeedItem[]> {
  // NEW: Use optimized repository pattern
  if (featureFlags.isEnabled('USE_REPOSITORY_PATTERN')) {
    const result = await feedRepository.getTrendingFeed(limit);
